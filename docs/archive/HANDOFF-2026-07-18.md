# Nimbus AI — Session Handoff (ARCHIVED SNAPSHOT, frozen 2026-07-18)

> **This is a historical archive, not a live doc.** It is a point-in-time
> narrative of everything done through 2026-07-18, kept for reference only.
> The live continuity doc is `DECISIONS.md` at the repo root — read that, not
> this. Nothing here is maintained; treat any claim as possibly stale and verify
> against current code and `git log`. (`context.md`, referenced below, was deleted
> as incorrect — ignore those references.)

---

## E2E bug batch — user's manual testing findings (2026-07-15, Fable 5)

User manually e2e-tested and found real problems. Root causes CONFIRMED by code +
live DB/AWS inspection, then fixed (suite 248/248 vs real Postgres, tsc clean):

1. **Surprise Bedrock bill** — two confirmed sources: (a) Bodyguard's patrol ran a
   Bedrock tool-loop (≤10 invocations) per connected user every 5 min, 24/7 —
   `get_provider()` defaulted to bedrock and patrols never passed a provider;
   (b) the frontend ModelSelector's initial state was "bedrock" until manually
   switched. **Fixed:** patrol is now fully deterministic plain code (same
   decision rules, zero LLM — `agents/bodyguard.py`); provider default is now
   `openrouter` everywhere (`utils/llm/get_provider`, chat.KNOWN_PROVIDERS drops
   bedrock, session default, frontend PROVIDERS list + localStorage migration);
   landing-footer provider claims removed. NOTE: only OPENROUTER_API_KEY exists in
   backend/.env — Groq/HF selector entries error with a clear message until keys
   are added. Bedrock is no longer user-selectable (BedrockProvider class kept for
   tests only). Also fixed: bodyguard daemon task had no strong reference (GC risk)
   and stop_bodyguard() now cancels it.
2. **"Delusional" deploy** — NOT a hallucination: the deployment row shows the
   executor ran only 1 of 3 plan steps (API Gateway `jrgjr1tofe` REALLY EXISTS in
   us-east-1 — user should check API Gateway console there and delete it if
   unwanted; HTTP APIs cost $0 idle). Lambda + IAM role steps were never executed,
   yet status read "success" because status only counted failures among RETURNED
   results. **Fixed:** `_reconcile_results` in `pipeline/orchestrator.py` turns
   every never-executed plan step into an explicit failure entry (status becomes
   partial/failed; validator+summary see the truth), and `_finalize_turn` appends
   a deterministic "N of M steps did not complete" warning independent of the LLM
   summary's narrative.
3. **Post-deploy amnesia** ("asks if I'm ready to deploy after I already did") —
   executor_node/cancel_node never wrote the executed/cancelled turn into agent
   history, and `_finalize_turn` never persisted history on those outcomes.
   **Fixed:** both nodes append a factual record; both branches persist.
4. **Sidebar toggle shifted the chat column** — the sidebar swapped w-64/w-14 in
   flex flow. **Fixed:** wrapper reserves w-64 in both states (messages are
   width-capped anyway, collapse bought nothing but the jump).
5. Regression tests added: bodyguard deterministic patrol (6), reconciliation +
   history persistence (3 in test_orchestrator).

**Still open from this batch (tasked):** E2E-1 token streaming of replies (biggest
UX complaint — replies feel interminable; stream final text word-by-word),
E2E-4 real costs via Cost Explorer (dashboard hardcodes $0 free-tier notes; user's
console shows ~$1 July incl. S3), E2E-6 resource detail view (click resource →
console-style info/logs + start/stop + danger delete w/ confirm; note the deploy
above created an API Gateway the dashboard can't even display — only EC2/S3/
DynamoDB/Lambda are listed).

## Production hardening — ACTIVE (kicked off 2026-07-15, Fable 5)

User explicitly greenlit ALL production work after a senior-eng critique of the old
PROD list. **Decisions made with the user:**
- **Deploy target (PROD-4): backend on free-tier EC2 + docker compose + Caddy
  reverse proxy; frontend to Vercel free tier.** K8s explicitly considered and
  rejected for now: single-node k3s doesn't fit in 1 GB free-tier RAM, gives zero
  extra survivability over compose restart policies on one machine, and everything
  the user wanted it for (ingress/reverse-proxy/request control) Caddy provides.
  Same Docker images migrate to ECS/EKS when traffic justifies multi-node.
- Revised task list (tracked in the session harness):
  - **PROD-0** CI + lockfile — **DONE 2026-07-15** (see below)
  - **PROD-1** Reliability groundwork — **DONE 2026-07-15** (see below)
  - **PROD-2R** Bodyguard durability — **DONE 2026-07-15** (see below)
  - **PROD-3** Redis — **DONE 2026-07-15**; **PROD-3.5** worker extraction —
    **DONE 2026-07-15**; **PROD-2** central config/validation + alembic release
    step — **DONE 2026-07-15**; **PROD-6** observability — **DONE 2026-07-15**;
    **PROD-5** daily spend caps — **DONE 2026-07-15**; **PROD-7** k6 — DONE
    earlier same day (re-measure on the EC2 box once it exists).
  - **PROD-4** deploy — **code/docs DONE 2026-07-15** (docker-compose.prod.yml,
    infra/Caddyfile, .github/workflows/deploy.yml, infra/DEPLOY.md); the
    remaining steps are USER actions: launch the EC2 box + DNS + secrets per
    infra/DEPLOY.md, import the repo into Vercel. Suite green after all of it:
    261+ passed vs disposable Postgres+Redis.

**PROD-2R shipped (2026-07-15):** Bodyguard alerts/logs/patrol-state moved from
process RAM to Postgres — tables `bodyguard_alerts`/`bodyguard_logs`/
`bodyguard_status` (migration `c41d8f0a92b7`, APPLIED to Supabase). Patrol
collects into a per-run buffer, loop persists via `_persist_patrol`; reads
(`get_status`/`get_alerts`/`mark_alert_read`) are async DB queries (same JSON
shapes — frontend untouched). Logs pruned after 7 days. Patrol now skips users
with zero Deployment rows (no Nimbus-managed resources can exist → saves
AssumeRole + 6 AWS calls/user/cycle). "running" in status = daemon active OR
last_check fresh (<2×interval), so it stays truthful with the worker split.

**PROD-3 + 3.5 shipped (2026-07-15):** `utils/redis_client.py` (REDIS_URL unset
⇒ every getter None ⇒ in-process fallbacks; Redis errors fail open, logged
once). Redis-backed: rate-limit token buckets (atomic Lua in `ratelimit.py`),
STS temp-cred L2 cache (`utils/aws_role.py`, JSON w/ TTL ending before cred
expiry), dashboard snapshot cache (`routes/dashboard.py`). Bodyguard extracted
to `backend/worker.py` (`python worker.py`, single replica, SIGTERM-clean);
API gates its in-process daemon on `BODYGUARD_IN_API` (default true for bare
dev runs; compose sets false). docker-compose.yml grew `redis` (64mb LRU,
healthchecked, not host-exposed) + `worker` services. CI runs a redis:7
service; Redis-path tests in `tests/integration/test_redis_backends.py` gate
on REDIS_TEST_URL (skip locally, run in CI).

**PROD-2 shipped (2026-07-15):** `backend/config.py::validate_environment(role)`
— aggregated fail-fast at boot for required env (DATABASE_URL, AWS service
creds; CLERK_ISSUER api-only) + parse-checks every numeric tunable; warns on
missing OPENROUTER_API_KEY/REDIS_URL. Called from main.py and worker.py.
Migrations are a release step: compose backend command = `alembic upgrade head
&& uvicorn ...` (worker deliberately doesn't migrate — no DDL races).
backend/.env.example now documents every knob.

**PROD-6 shipped (2026-07-15):** `backend/observability.py` — JSON logs
(LOG_FORMAT=json in containers, text for dev), X-Request-ID honored/generated/
echoed + stamped on every log line via contextvar, Prometheus `/metrics`
(requests + latency histograms labeled by ROUTE TEMPLATE, buckets to 1800s for
chat turns), Sentry init only when SENTRY_DSN set (traces off). Outermost
middleware so 401s/429s are counted too. `/metrics` blocked at Caddy in prod.

**PROD-5 shipped (2026-07-15):** `backend/budget.py` — per-user daily chat-turn
cap (DAILY_TURN_LIMIT, default 100/day, 0 disables; conftest + loadtest set 0),
Redis-shared counters w/ in-process fallback, denied attempts don't burn
budget. Both /chat endpoints 429 with reset-at-midnight-UTC Retry-After before
any LLM/AWS work starts.

**PROD-4 code shipped (2026-07-15):** `docker-compose.prod.yml` (Caddy owns
80/443; backend not host-exposed; worker+redis; ACME certs in a volume),
`infra/Caddyfile` (auto-HTTPS, SSE flush_interval -1, 1900s proxy timeouts >
TURN_TIMEOUT, /metrics 403'd, security headers), `.github/workflows/deploy.yml`
(runs on CI success on main or manual dispatch; ssh → git reset --hard →
compose up -d --build → readiness gate; needs repo secrets DEPLOY_HOST/
DEPLOY_USER/DEPLOY_SSH_KEY), `infra/DEPLOY.md` (EC2 one-time setup incl. swap,
Elastic IP, auto-recovery alarm, Vercel import steps, ops crib sheet).

**PROD-7 shipped (2026-07-15):** `loadtest/` — isolated k6 harness (own tmpfs
Postgres, mock OpenAI-compatible LLM with a latency knob, local JWKS issuer that
mints real RS256 JWTs so every VU is a distinct authenticated user; nothing
touches Supabase/Clerk/AWS/paid providers). One code change: `OPENROUTER_BASE_URL`
env override in `utils/llm/__init__.py` so the harness can aim the openrouter
provider at the mock. Scenarios: smoke, baseline+capacity (encodes the PROD-1
guarantees as thresholds), SSE soak. **First baseline recorded in
loadtest/README.md** — headline: reads p95 7ms during full turn saturation
(executor isolation proven), ~24ms pipeline overhead over model latency, 0
nominal rejections, saturation sheds load instantly via 503+Retry-After, 1,124/
1,124 SSE streams reached a terminal event. Re-measure on the EC2 target at
PROD-4 time. `loadtest/out/` (minted tokens) is gitignored.

**PROD-0 shipped:** `.github/workflows/ci.yml` (backend pytest vs a postgres:16
service container — the integration tests that always skip locally RUN in CI;
frontend tsc + next build with a format-valid dummy Clerk key, overridable via repo
secret `NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY`; both Docker images build).
`backend/requirements.lock.txt` — 226 pins generated by pip-compile INSIDE
python:3.11-slim (Linux-only uvloop makes Windows-generated locks wrong);
backend/Dockerfile now installs from the lock. Regenerate the lock via the same
docker run when requirements.txt changes.

**PROD-1 shipped:** (a) `db/engine.py` — pool_pre_ping/pool_recycle=1800/sized
pool (DB_POOL_SIZE/DB_MAX_OVERFLOW), fail-fast on missing DATABASE_URL.
(b) `utils/aws_clients.make_client()` — every boto3 client in the codebase now
carries connect/read timeouts + standard-mode retries (bedrock-runtime gets
read_timeout=300); cloud_control/aws_dispatch/pricing_api/aws_role/settings all
routed through it. (c) `routes/chat.py` — dedicated bounded turn executor
(MAX_CONCURRENT_TURNS, default 8) + semaphore admission (immediate 503 +
Retry-After at capacity, both endpoints) + TURN_TIMEOUT_SECONDS hang guard
(default 1800 → 504). Turns can no longer starve the default to_thread pool that
the dashboard uses. (d) `main.py` lifespan drains in-flight `_turn_tasks` on
shutdown (SHUTDOWN_DRAIN_SECONDS=55; compose stop_grace_period=60s) — deploys no
longer kill mid-deploy turns. (e) SSE: 15s keep-alive comments + Cache-Control:
no-cache + X-Accel-Buffering: no. (f) `/health/ready` checks the DB (503 when
unreachable); `/health` stays a dumb liveness probe.

**Test-infra bug found by the CI rehearsal (fixed):** the integration `client`
fixture drives the real app as ONE user, so SEC-2's per-user rate limiter 429'd
13 integration tests partway through a full-suite run — invisible since 2026-07-09
because those tests always skipped without a local Postgres. `tests/conftest.py`
now force-sets RATE_LIMIT_*_PER_MINUTE huge before app import (limiter keeps its
own unit tests).

**Clerk Billing — what the USER must do (code is ready and waiting):** see the
"Clerk Billing + backend claim checks" section below; short version: Clerk
Dashboard → Billing → enable + create plan(s) with slugs/prices/features, then
tell the assistant the slugs + gating rules to wire `require_plan`/`require_feature`.

## Hero redesign — DONE (2026-07-12, executed by Fable 5)

Implemented exactly per the plan below (Sonnet researched/planned it 2026-07-11;
Fable 5 executed 2026-07-12). What shipped, all in the working tree (uncommitted):
- **`frontend/app/components/LoomBackground.tsx`** (new) — Canvas2D node/thread
  mesh (44 nodes, 210px link distance, ion-blue, DPR-aware, radial edge-fade
  mask, wrap-around drift). `prefers-reduced-motion` renders one static settled
  frame instead of animating (verified via Playwright `emulateMedia`).
- **`frontend/app/page.tsx`** — `HyperdriveClouds`/wisps deleted; headline now
  monochrome pure-white `text-6xl/7xl font-extrabold` single sentence ("Deploy
  AWS in plain English.") with the gradient span dropped; hero + navbar CTAs are
  `rounded-full` pills with trailing Phosphor `ArrowRight` (hover nudges the
  arrow); badge tightened to a small bordered pill with an ion dot. Asymmetric
  split + `ChatPreview` kept, per plan.
- **Verified**: screenshots at 1440px and 390px (mobile stacks cleanly, headline
  wraps to three centered lines), reduced-motion static frame confirmed,
  `npx tsc --noEmit` clean, `npm run build` green.
- Remember: the Docker image needs `docker compose build && docker compose up -d`
  to show this — `up` alone serves the stale image.

The original plan is kept below as the design record.

**Also fixed by Sonnet this same session (already committed/verified, unrelated to
the hero — don't redo):** three Clerk theming bugs (invisible gray-on-dark text in
the login "last used" badge, the UserButton popover name/email, and the
`<PricingTable />` card) — see `app/layout.tsx`'s `appearance.elements` for
`badge`, `userPreviewMainIdentifier`/`userPreviewSecondaryIdentifier`, and the
`pricingTableCard*` keys. Also stood up real Playwright e2e infra with actual
authenticated browser sessions (`frontend/e2e/`, see the section below) and fixed
a live UI bug where `docker compose up` alone (no `--build`) was serving code
that was **3 days stale** — any time Docker needs to reflect new code, it's
`docker compose build && docker compose up -d`, not just `up`.

### The three references (already fetched/screenshotted this session — don't re-fetch)

1. **nextjs.org** — centered layout, huge/bold headline, generous whitespace,
   near-zero background decoration in the hero itself (3D visuals live *below*
   the fold, not in it), restraint over flash, dual CTA links.
2. **21st.dev `@dhiluxui/digital-loom-background`** — an animated background
   component. Key fact: this is **Canvas2D, not WebGL/Three.js**. Don't add
   Three.js as a dependency for this — it isn't earning its weight here.
3. **Dribbble "Saas Landing page design" (shot 27540722, the "Lamun" mock by
   Lalit for theosm)** — the strongest signal. Near-black bg, a few soft diagonal
   violet-blue light beams (not a radiating starburst), a small bordered pill
   badge above the headline, a **huge, bold, pure-white headline with zero
   gradient** (two tight lines, no color at all on the type), muted gray
   subheading, and **pill-shaped CTA buttons with trailing arrow icons** (one
   solid white-on-black, one outlined). Extremely restrained — the only color in
   the whole hero is a whisper of blue-violet ambient light.

### The plan (drafted, not built)

- **Keep the asymmetric split layout** (copy left, real chat/plan-preview right)
  — the `ChatPreview` component showing actual product UI is the one genuinely
  non-generic asset already on the page (real plan card, real bubble styling) and
  is worth keeping as the hero's visual per the "hero is a thesis" principle. Do
  NOT flatten to a single centered column just because two of the three
  references are centered — the product screenshot earns the split.
- **Headline**: go much bigger/bolder (references are dramatically larger than
  the current `text-5xl/6xl`), tighter leading, and **drop the gradient span** —
  match the reference's near-monochrome white treatment. If keeping any accent
  color at all, keep it extremely restrained (a solid color on one word, not a
  gradient/shimmer) — this is a bigger departure than earlier de-templating passes
  made and is worth actually trying per the reference, not diluting.
- **Signature element**: replace the current `HyperdriveClouds` component
  (SVG wisps radiating from a center point — already toned down once this
  session but still the wrong shape entirely per the new reference) with a
  Canvas2D animated node/thread mesh in the existing ion-blue palette. Make the
  metaphor literal to Nimbus, not borrowed decoration: nodes are
  resources/agents, threads are the connections between them, redrawing as the
  graph reconfigures — directly ties to "three agents managing your
  infrastructure," not generic ambient art. Must respect
  `prefers-reduced-motion` (render one static settled frame instead of
  animating). A from-scratch attempt at this (`components/LoomBackground.tsx`,
  requestAnimationFrame-driven node/link mesh) was written and then reverted
  when the user redirected to Fable — reference it in git history
  (`git show HEAD:frontend/app/components/LoomBackground.tsx` won't work since
  it was never committed; check `git stash list` / recent Sonnet session
  transcript if the code itself is wanted as a starting point) or just rebuild
  it, it's a small file.
- **CTAs**: pill-shaped (`rounded-full`, not the current `rounded-2xl`) with a
  trailing arrow icon (Phosphor `ArrowRight` is already a project dependency via
  `@phosphor-icons/react`), matching the reference exactly.
- **Badge pill** above the headline: tighten to match the reference's smaller,
  more restrained sizing.
- Everything else on the page (agents section, how-it-works, CTA block, footer)
  was already de-templated this session (static gradient instead of shimmer,
  honest stats, plain section headers) — don't redo that pass, just the hero.

### Verify before calling it done

- Screenshot at desktop (1440px) and mobile (390px) — this project has a working
  `gstack browse` setup and/or the Playwright infra below; use either.
  `prefers-reduced-motion` must be checked (`emulateMedia` in Playwright or the
  OS setting) since the whole point of the canvas signature is that it degrades
  to a static frame, not that it's removed.
- `npx tsc --noEmit` and `npm run build` clean, per this project's standing rule.

### Everything else still open (after the hero — same priority as the Task List further down, consolidated here so it's all in one place)

**UI/UX** (details + rationale in the "Design/UX" block of the Task List below):
- **UX-2** — Assistant chat replies render markdown as literal text (`**bold**`,
  list markers, code fences all show as raw characters instead of formatting).
- **UX-3** — `/terminal` page is orphaned (not linked from the navbar, duplicates
  the Dashboard's Bodyguard panel). Needs a decision: fold into Dashboard, or cut.
- **UX-4** — Surface hierarchy pass: every card across the app uses the identical
  flat `glass` treatment (no base/elevated/floating layering), and the type scale
  between hero-level and card-label-level text is timid. Broader than the hero.
- **UX-5** — Failed/errored chat turns aren't persisted to `ui_messages`, so
  reloading a session silently drops them from history.

**Billing — blocked on a business decision, not code:**
- Confirmed live this session: only the "Free" plan exists in Clerk. The
  Settings → Billing page (`<PricingTable />`, now readable after tonight's
  contrast fix) currently shows exactly one plan, which reads as broken/unfinished
  even though the code is correct. **Someone needs to define the actual paid
  tier(s)** (slug names, price, what's gated) in Clerk Dashboard → Billing before
  there's anything more to build here — the backend gating plumbing
  (`require_plan`/`require_feature` in `backend/auth.py`) already exists and is
  tested, it's just not attached to any route because no rule has been decided.

**Production readiness** (ordering matters — see REVIEW.md ENG-8 for why):
PROD-3 (Redis) → PROD-3.5 (extract Bodyguard to its own process) → PROD-4
(Kubernetes). Don't skip ahead to K8s manifests while Bodyguard is still an
in-process daemon and the rate limiter / STS role cache are still process-local
— running 2 replicas today would mean duplicate Bodyguard patrols and split rate
budgets. PROD-2 (deployment target/env management), PROD-5 (broader
error-handling/validation pass), PROD-6 (Grafana/Prometheus), PROD-7 (k6 load
tests) are all still fully open, no work started.

**Testing:**
- **TEST-7** — Real Playwright infra now exists and passes (`frontend/e2e/`, 9
  tests: landing page, unauthenticated route protection, and — new this session —
  genuine authenticated coverage via `@clerk/testing`'s Agent Task / real sign-in
  flow, session cached in `playwright/.clerk/user.json`). But coverage is still
  thin: only page-reachability is tested, nothing exercises the actual chat turn
  flow (send message → plan proposed → confirm/cancel → execution), settings
  forms, or the SSE stream. That's the highest-value next slice.
  **Prerequisite everyone needs to know:** Clerk's "Client Trust" (new-device
  email verification) had to be disabled in Clerk Dashboard → Configure → Attack
  protection for automated sign-in to work at all — already done, don't re-enable
  it without also updating the e2e auth flow to handle the resulting verification
  step.
